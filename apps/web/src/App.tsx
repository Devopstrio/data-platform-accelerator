...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Rocket, Layers, ClipboardCheck, DollarSign } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Data Launchpad</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/platforms" icon={<Layers size={20} />} label="Platform Explorer" />
            <NavItem to="/onboarding" icon={<Zap size={20} />} label="Rapid Onboarding" />
            ...
            <NavItem to="/costs" icon={<DollarSign size={20} />} label="Cost Management" />
            <NavItem to="/resilience" icon={<Globe size={20} />} label="Global Resilience" />
          </nav>

          <div className="pt-6 border-t border-slate-800">
            <NavItem to="/settings" icon={<Settings size={20} />} label="Accelerator Settings" />
          </div>
        </aside>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              <span>Platform Accelerator</span>
              <span>/</span>
              <span className="text-white font-bold">Orchestration Control Plane</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Chief Platform Architect</p>
                <p className="text-[10px] text-indigo-400 uppercase tracking-widest font-black">Industrialized Data</p>
              </div>
